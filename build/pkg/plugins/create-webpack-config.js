...
export default ()=> {
  const buildEntries = [${(0, _utils.parseArrayString)(answers.buildEntries).array.map(target => `'${target}'`).toString()}];
  const buildOutputPath = '${answers.buildOutputPath}';
  const devServerPort = ${answers.devServerPort};
  const faviconUrl = '${(0, _isurl.default)(answers.faviconUrl) || answers.faviconUrl.startsWith('http') ? 'favicon.png' : answers.faviconUrl}';
