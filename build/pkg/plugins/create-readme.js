...
const createReadmeHeader = store => {
  ...
  if ((0, _isurl.default)(faviconUrl) || faviconUrl.startsWith('http')) {
    src = faviconUrl;
  }
...
${store.answers.useCodeCoverage ? 'You will find code coverage results in `build/cov` including a HTML report:\n```bash\n open ./build/cov/lcov-report/index.html \n```\n' : ''}
...
${store.answers.repoMaintainers.split(',').map(maintainer => `[@${maintainer}](https://github.com/${maintainer}) \n`).toString()}
`;
...
const createReadme = async store => {
  const task = {
    type: 'batch',
  ...
  store.addTask({
    ...
    children: [{
      ...
      task: storeCtx => {
        _fs.default.writeFileSync(`${storeCtx.workingDir}/README.md`, storeCtx.readme);
